    ...
    <div className="rounded-lg border border-border bg-muted/30 overflow-hidden mx-2">
      ...
      {isExpanded && (
        <div className="border-t border-border">
          ...
          {plan.status === "awaiting_approval" && (
            <div className="px-2.5 py-2 border-t border-border bg-muted/50 flex items-center justify-between gap-2">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  // Send message to implement the plan
                  const event = new CustomEvent('send-chat-message', {
                    detail: { message: 'Please implement the plan above.' }
                  })
          ...
          {plan.status === "approved" && completedCount < totalSteps && (
            <div className="px-2.5 py-2 border-t border-border bg-muted/50 flex items-center justify-between gap-2">
              <span className="text-xs text-muted-foreground">
                Continue implementing this plan
              </span>
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  // Send message to continue implementation
                  const event = new CustomEvent('send-chat-message', {
                    detail: { message: 'Please continue implementing the plan.' }
                  })
                  window.dispatchEvent(event)
                }}
                className="px-3 py-1.5 text-xs font-medium bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors"
